...
class DatabaseService:
    ...
    @staticmethod
    async def get_last_history_id_with_lock(conn) -> Optional[int]:
        """Get last history ID with row lock (must be called within transaction)"""
        async with conn.cursor() as cursor:
            await cursor.execute("""
                SELECT last_history_id 
                FROM gmail_history_tracker 
    ...
    @staticmethod
    async def store_history_id_in_transaction(conn, history_id: int):
        """Store history ID within existing transaction, maintaining max 100 records"""
        async with conn.cursor() as cursor:
            # Get current count of records
            await cursor.execute("SELECT COUNT(*) FROM gmail_history_tracker")
            current_count_result = await cursor.fetchone()
            current_count = current_count_result[0] if current_count_result else 0

            # If we have 100 or more records, delete the oldest ones to maintain max 100
            if current_count >= 100:
                records_to_delete = current_count - 99  # Keep space for the new record

                await cursor.execute(
                    """
                    DELETE FROM gmail_history_tracker 
                    WHERE id IN (
                        SELECT id 
                        FROM gmail_history_tracker 
                ...
                logger.info(
                    f"Deleted {records_to_delete} oldest history records to maintain limit"
                )

            # Insert new record
            await cursor.execute(
                """
                INSERT INTO gmail_history_tracker (last_history_id) 
                VALUES (%s)
            """,
                (history_id,),
            )

            logger.info(f"Stored new history ID: {history_id}")
