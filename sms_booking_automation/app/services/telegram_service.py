...
class TelegramService:
    def __init__(self):
        ...
        self.target_chat_ids = [
            chat_id.strip()
            for chat_id in TELEGRAM_TARGET_CHAT_IDS.split(",")
            if chat_id.strip()
        ]
    ...
    def _send_request(self, method: str, payload: dict) -> dict:
        ...
        try:
            ...
            logger.error(
                f"Telegram API HTTP error for method '{method}': {e.response.status_code} - {e.response.text}"
            )
            raise TelegramAPIError(f"Telegram API error: HTTP {e.response.status_code}")
        except requests.exceptions.RequestException as e:
            logger.error(
                f"Network error calling Telegram API method '{method}': {str(e)}"
            )
            raise TelegramServiceError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error(
                f"Unexpected error calling Telegram API method '{method}': {str(e)}"
            )
    ...
    def send_message_to_targets(self, text: str, photo_url: Optional[str] = None):
        ...
        if errors:
            raise TelegramServiceError(
                f"Failed to send to some targets: {'; '.join(errors)}"
            )
